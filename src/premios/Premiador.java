...
public final class Premiador {
    ...
    public static boolean hayClubesValidos(Club[] clubes) {
        if (clubes == null) return false;
        int count = 0;
        for (int i = 0; i < clubes.length; i++) {
            if (clubes[i] != null) count++;
        }
        return count > 0;
    }
    ...
    public static PremioJugador[] calcularPremios(Club[] clubes) {
        ...
        if (!hayClubesValidos(clubes)) return premios;
    ...
    private static int valorPorTipo(Jugador j, TipoPremio tipo) {
        ...
        if (tipo == TipoPremio.PICHICHI) return j.getGoles();
        if (tipo == TipoPremio.ASISTENCIAS) return j.getAsistencias();
        if (tipo == TipoPremio.ZAMORA) {
            if (j.getPosicion() != Posicion.POR) return -1;
            return j.getPorteriasCero();
        }
        if (tipo == TipoPremio.MVP) return j.getValoracion();
        return 0;
    }
