...
for (int i = 0; i < partidos.length; i++) {
    ...
    if (eliminatoria == null) continue;

    if (rondaEnCurso >= 3) {
        if (eliminatoria.isJugado()) continue;
    ...
    private Club resolverGanador(Partido partido) {
        ...
        if (gl > gv) return partido.getEquipoLocal();
        if (gv > gl) return partido.getEquipoVisitante();

        int x = rnd.nextInt(2);
        if (x == 0) return partido.getEquipoLocal();
    ...
    private String nombreRonda(int r) {
        if (r == 0) return "OCTAVOS";
        if (r == 1) return "CUARTOS";
...
public String[] listarPartidosDisponibles() {
    ...
    if (haTerminado()) return new String[0];
    if (rondas == null || rondas.length == 0) return new String[0];
    if (rondaEnCurso < 0 || rondaEnCurso >= rondas.length) return new String[0];

    Jornada ronda = rondas[rondaEnCurso];
    if (ronda == null) return new String[0];

    Partido[] ps = ronda.getPartidos();
    if (ps == null || ps.length == 0) return new String[0];

    int total = 0;
    for (int i = 0; i < ps.length; i++) {
        if (ps[i] == null) continue;
        if (ps[i].isJugado()) continue;
        total++;
    }
...
public boolean simularPartidoEnVivo(int indiceSeleccion) {
    ...
    if (rondas == null || rondas.length == 0) return false;
    if (rondaEnCurso < 0 || rondaEnCurso >= rondas.length) return false;
    ...
    if (indiceSeleccion > total) indiceSeleccion = total;
    ...
    Partido[] unSolo = new Partido[1];
    unSolo[0] = elegido;

    for (int i = 0; i < unSolo.length; i++) {
    ...
    if (!quedan) {
        ...
        if (rondaEnCurso >= rondas.length) {
            setTerminada(true);
            if (numGanadoresUltimaRonda > 0) campeon = ganadoresUltimaRonda[0];
            Club c = campeon;
            if (c != null) {
                System.out.println("\nCAMPEON: " + c.getNombre());
                c.ganarChampions();
